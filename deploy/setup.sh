...
# 2. Python 가상환경 + 의존성 (argus-ocr: 메인 FastAPI + MLX)
cd "$ARGUS_DIR"
if [ ! -d ".venv" ]; then
...
if [ -d "$NGINX_SITES" ]; then
    ...
    echo "[6/9] Nginx servers 디렉토리를 찾을 수 없음 — 수동으로 $ARGUS_DIR/deploy/nginx.conf 적용 필요"
fi
...
echo "[8/9] Cloudflare Tunnel 설정 안내:"
